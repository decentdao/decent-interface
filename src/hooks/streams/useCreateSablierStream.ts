...
export default function useCreateSablierStream() {
  const {
    contracts: { sablierV2LockupDynamic, sablierV2Batch },
  } = useNetworkConfig();
  const { node: { daoAddress }} = useFractal();
  ...
  const prepareCreateDynamicLockupProposal = useCallback(
    ({
      ...
      const segments: { amount: bigint; exponent: bigint; duration: number }[] = [{ amount: 0n, exponent, duration: Math.round(startDate - (Date.now() / 1000))}];
  ...
  return { prepareCreateDynamicLockupProposal };
}
