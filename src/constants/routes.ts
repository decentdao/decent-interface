export const BASE_ROUTES = {
  landing: '/',
  create: '/create',
};

type RouteInfo = { relative: (...args: any) => string; path: string };
type RouteIndex = { [key: string]: RouteInfo };
export interface DAORoutes extends RouteIndex {
  dao: RouteInfo;
  daos: RouteInfo;
  newSubDao: RouteInfo;
  hierarchy: RouteInfo;
  treasury: RouteInfo;
  proposals: RouteInfo;
  proposal: RouteInfo;
  proposalNew: RouteInfo;
  integrations: RouteInfo;
  integration: RouteInfo;
  integrationNew: RouteInfo;
}

export const DAO_ROUTES: DAORoutes = {
  daos: {
    relative: () => '/daos',
    path: 'daos/:address/*',
  },
  dao: {
    relative: (daoAddress: string) => `/daos/${daoAddress}`,
    path: '*',
  },
  newSubDao: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/new`,
    path: 'new',
  },
  hierarchy: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/hierarchy`,
    path: 'hierarchy',
  },
  treasury: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/treasury`,
    path: 'treasury',
  },
  proposals: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/proposals`,
    path: 'proposals',
  },
  proposal: {
    relative: (daoAddress: string, proposalNumber: string) =>
      `/daos/${daoAddress}/proposals/${proposalNumber}`,
    path: 'proposals/:proposalNumber',
  },
  proposalNew: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/proposals/new`,
    path: 'proposals/new',
  },
  integrations: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/integrations`,
    path: 'integrations',
  },
  integration: {
    relative: (daoAddress: string, integrationKey: string) =>
      `/daos/${daoAddress}/integrations/${integrationKey}`,
    path: 'integrations/:integrationKey',
  },
  integrationNew: {
    relative: (daoAddress: string) => `/daos/${daoAddress}/integrations/new`,
    path: 'integrations/new',
  },
};
